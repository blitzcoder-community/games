...
//${METADATA_BEGIN}
var META_DATA="[textures.png];type=image/png;width=512;height=256;\n[mojo_font.png];type=image/png;width=864;height=13;\n";
//${METADATA_END}
...
function c_myApp(){
	c_App.call(this);
	this.m_worldMap=new_number_array(384);
	this.m_gfx=null;
	this.m_light=new_object_array(8);
	this.m_dark=new_object_array(8);
	this.m_tiles=null;
...
c_myApp.prototype.p_OnCreate=function(){
	var t_levelData="";
	t_levelData=t_levelData+"777777777777222224444444";
	t_levelData=t_levelData+"700000000007000004000004";
	t_levelData=t_levelData+"700000000007444444000004";
	t_levelData=t_levelData+"700022120000000000000004";
	t_levelData=t_levelData+"700010020007440444444444";
	t_levelData=t_levelData+"700020010007040400000004";
	t_levelData=t_levelData+"700021220007040000000004";
	t_levelData=t_levelData+"700000000007044444444444";
	t_levelData=t_levelData+"700000000007007777777772";
	t_levelData=t_levelData+"777007777777007000000002";
	t_levelData=t_levelData+"700000007000007000000002";
	t_levelData=t_levelData+"770777777000007000055502";
	t_levelData=t_levelData+"200070007777777000055502";
	t_levelData=t_levelData+"200070000000000000055502";
	t_levelData=t_levelData+"200000007777777000000002";
	t_levelData=t_levelData+"222222222222222222222222";
	for(var t_index=0;t_index<t_levelData.length;t_index=t_index+1){
		this.m_worldMap[t_index]=parseInt((t_levelData.slice(t_index,t_index+1)),10);
	}
	this.m_gfx=bb_graphics_LoadImage("textures.png",1,c_Image.m_DefaultFlags);
	for(var t_index2=0;t_index2<8;t_index2=t_index2+1){
		this.m_light[t_index2]=this.m_gfx.p_GrabImage(t_index2*64,0,1,64,64,c_Image.m_DefaultFlags);
		this.m_dark[t_index2]=this.m_gfx.p_GrabImage(t_index2*64,64,1,64,64,c_Image.m_DefaultFlags);
	}
	this.m_tiles=this.m_gfx.p_GrabImage(0,0,64,64,8,c_Image.m_DefaultFlags);
	this.m_background=this.m_gfx.p_GrabImage(0,128,128,128,1,c_Image.m_DefaultFlags);
	bb_app_SetUpdateRate(30);
	return 0;
}
c_myApp.prototype.p_WorldIndex=function(t_x,t_y){
	return 24*t_y+t_x;
}
c_myApp.prototype.p_OnRender=function(){
	bb_graphics_DrawImage2(this.m_background,0.0,0.0,0.0,5.0,3.75,0);
	for(var t_x=0;t_x<640;t_x=t_x+1){
		this.m_cameraX=(2*t_x)/640.0-1.0;
		this.m_rayPosX=this.m_posX;
		this.m_rayPosY=this.m_posY;
		this.m_rayDirX=this.m_dirX+this.m_planeX*this.m_cameraX;
		this.m_rayDirY=this.m_dirY+this.m_planeY*this.m_cameraX;
		this.m_mapX=((this.m_rayPosX)|0);
		this.m_mapY=((this.m_rayPosY)|0);
		this.m_deltaDistX=Math.sqrt(1.0+this.m_rayDirY*this.m_rayDirY/(this.m_rayDirX*this.m_rayDirX));
		this.m_deltaDistY=Math.sqrt(1.0+this.m_rayDirX*this.m_rayDirX/(this.m_rayDirY*this.m_rayDirY));
		this.m_hit=false;
		this.m_side=false;
		if(this.m_rayDirX<0.0){
			this.m_stepX=-1;
			this.m_sideDistX=(this.m_rayPosX-(this.m_mapX))*this.m_deltaDistX;
		}else{
			this.m_stepX=1;
			this.m_sideDistX=((this.m_mapX)+1.0-this.m_rayPosX)*this.m_deltaDistX;
		}
		if(this.m_rayDirY<0.0){
			this.m_stepY=-1;
			this.m_sideDistY=(this.m_rayPosY-(this.m_mapY))*this.m_deltaDistY;
		}else{
			this.m_stepY=1;
			this.m_sideDistY=((this.m_mapY)+1.0-this.m_rayPosY)*this.m_deltaDistY;
		}
		while(this.m_hit==false){
			if(this.m_sideDistX<this.m_sideDistY){
				this.m_sideDistX=this.m_sideDistX+this.m_deltaDistX;
				this.m_mapX=this.m_mapX+this.m_stepX;
				this.m_side=false;
			}else{
				this.m_sideDistY=this.m_sideDistY+this.m_deltaDistY;
				this.m_mapY=this.m_mapY+this.m_stepY;
				this.m_side=true;
			}
			if(this.m_worldMap[this.p_WorldIndex(this.m_mapX,this.m_mapY)]>0){
				this.m_hit=true;
			}
		}
		if(this.m_side==false){
			this.m_perpWallDist=bb_math_Abs2(((this.m_mapX)-this.m_rayPosX+(((1-this.m_stepX)/2)|0))/this.m_rayDirX);
		}else{
			this.m_perpWallDist=bb_math_Abs2(((this.m_mapY)-this.m_rayPosY+(((1-this.m_stepY)/2)|0))/this.m_rayDirY);
		}
		this.m_lineHeight=(bb_math_Abs((480.0/this.m_perpWallDist)|0));
		this.m_drawStart=-this.m_lineHeight/2.0+240.0;
		this.m_drawScale=this.m_lineHeight/64.0;
		this.m_texNum=this.m_worldMap[this.p_WorldIndex(this.m_mapX,this.m_mapY)]-1;
		if(this.m_texNum>-1 && this.m_drawScale>0.0){
			if(this.m_side==true){
				this.m_wallX=this.m_rayPosX+((this.m_mapY)-this.m_rayPosY+(((1-this.m_stepY)/2)|0))/this.m_rayDirY*this.m_rayDirX;
			}else{
				this.m_wallX=this.m_rayPosY+((this.m_mapX)-this.m_rayPosX+(((1-this.m_stepX)/2)|0))/this.m_rayDirX*this.m_rayDirY;
			}
			this.m_wallX-=Math.floor(this.m_wallX);
			this.m_texX=((this.m_wallX*64.0)|0);
			if(this.m_side==false && this.m_rayDirX>0.0){
				this.m_texX=64-this.m_texX-1;
			}
			if(this.m_side==true && this.m_rayDirY<0.0){
				this.m_texX=64-this.m_texX-1;
			}
			if(this.m_side==true){
				bb_graphics_DrawImage2(this.m_dark[this.m_texNum],(t_x),this.m_drawStart,0.0,1.0,this.m_drawScale,this.m_texX);
			}else{
				bb_graphics_DrawImage2(this.m_light[this.m_texNum],(t_x),this.m_drawStart,0.0,1.0,this.m_drawScale,this.m_texX);
			}
		}
	}
	bb_graphics_Translate(16.0,16.0);
	bb_graphics_SetColor(255.0,255.0,255.0);
	bb_graphics_DrawRect(0.0,0.0,150.0,102.0);
	bb_graphics_Translate(3.0,3.0);
	for(var t_y=0;t_y<16;t_y=t_y+1){
		for(var t_x2=0;t_x2<24;t_x2=t_x2+1){
			this.m_texNum=this.m_worldMap[this.p_WorldIndex(t_x2,t_y)]-1;
			if(this.m_texNum>-1){
				bb_graphics_DrawImage2(this.m_tiles,(t_x2)*6.0,(t_y)*6.0,0.0,0.09375,0.09375,this.m_texNum);
			}
		}
	}
	bb_graphics_SetColor(255.0,0.0,0.0);
	bb_graphics_DrawRect(this.m_posX*6.0-3.0,this.m_posY*6.0-3.0,6.0,6.0);
	return 0;
}
c_myApp.prototype.p_OnUpdate=function(){
	this.m_oldTime=this.m_time;
	this.m_time=bb_app_Millisecs();
	this.m_frameTime=(this.m_time-this.m_oldTime)/1000.0;
	this.m_moveSpeed=this.m_frameTime*4.0;
	this.m_rotSpeed=this.m_frameTime*90.0;
	if((bb_input_KeyDown(38))!=0){
		if(this.m_worldMap[this.p_WorldIndex(((this.m_posX+this.m_dirX*this.m_moveSpeed)|0),((this.m_posY)|0))]==0){
			this.m_posX=this.m_posX+this.m_dirX*this.m_moveSpeed;
		}
		if(this.m_worldMap[this.p_WorldIndex(((this.m_posX)|0),((this.m_posY+this.m_dirY*this.m_moveSpeed)|0))]==0){
			this.m_posY=this.m_posY+this.m_dirY*this.m_moveSpeed;
		}
	}
	...
	if((bb_input_KeyDown(39))!=0){
		this.m_oldDirX=this.m_dirX;
		this.m_dirX=this.m_dirX*Math.cos((-this.m_rotSpeed)*D2R)-this.m_dirY*Math.sin((-this.m_rotSpeed)*D2R);
		this.m_dirY=this.m_oldDirX*Math.sin((-this.m_rotSpeed)*D2R)+this.m_dirY*Math.cos((-this.m_rotSpeed)*D2R);
		this.m_oldPlaneX=this.m_planeX;
		this.m_planeX=this.m_planeX*Math.cos((-this.m_rotSpeed)*D2R)-this.m_planeY*Math.sin((-this.m_rotSpeed)*D2R);
		this.m_planeY=this.m_oldPlaneX*Math.sin((-this.m_rotSpeed)*D2R)+this.m_planeY*Math.cos((-this.m_rotSpeed)*D2R);
	}
...
c_Image.prototype.p_GrabImage=function(t_x,t_y,t_width,t_height,t_nframes,t_flags){
	...
	return (c_Image.m_new.call(new c_Image)).p_Init2(this.m_surface,t_x,t_y,t_width,t_height,t_nframes,t_flags,this,this.m_frames[0].m_x,this.m_frames[0].m_y,this.m_width,this.m_height);
}
