...
function c_PhotonApp(){
	c_App.call(this);
	this.m_render=null;
	this.m_sc=null;
}
c_PhotonApp.prototype=extend_class(c_App);
c_PhotonApp.m_new=function(){
	c_App.m_new.call(this);
	return this;
}
c_PhotonApp.prototype.p_OnCreate=function(){
	bb_app_SetUpdateRate(1280000);
	this.m_render=c_PhotonRender.m_new.call(new c_PhotonRender);
	this.m_sc=c_Scene.m_new.call(new c_Scene);
	this.m_render.p_SetScene(this.m_sc,this.m_sc.m_szImg,this.m_sc.m_szImg+48);
	return 0;
}
c_PhotonApp.m_mouseDragging=0;
c_PhotonApp.m_sphereIndex=0;
c_PhotonApp.m_s=0;
c_PhotonApp.prototype.p_GrabObject=function(){
	c_PhotonApp.m_sphereIndex=2;
	var t_mx=(bb_input_MouseX()-(this.m_sc.m_szImg)/2.0)/c_PhotonApp.m_s;
	var t_my=-(bb_input_MouseY()-(this.m_sc.m_szImg)/2.0)/c_PhotonApp.m_s;
	var t_mouse3=[t_mx,t_my,0.5*(this.m_sc.m_spheres[0][2]+this.m_sc.m_spheres[1][2])];
	if((bb_raytracer_gatedSqDist3(t_mouse3,this.m_sc.m_spheres[0],this.m_sc.m_spheres[0][3]))!=0.0){
		c_PhotonApp.m_sphereIndex=0;
	}else{
		if((bb_raytracer_gatedSqDist3(t_mouse3,this.m_sc.m_spheres[1],this.m_sc.m_spheres[1][3]))!=0.0){
			c_PhotonApp.m_sphereIndex=1;
		}
	}
	print("sphere:"+String(c_PhotonApp.m_sphereIndex));
	c_PhotonApp.m_mouseDragging=1;
	return 0;
}
c_PhotonApp.m_prevMouseX=0;
c_PhotonApp.m_prevMouseY=0;
c_PhotonApp.prototype.p_DragObject=function(){
	if(c_PhotonApp.m_prevMouseX>-9999 && c_PhotonApp.m_sphereIndex>-1){
		if(c_PhotonApp.m_sphereIndex<this.m_sc.m_nrObjects[0]){
			this.m_sc.m_spheres[c_PhotonApp.m_sphereIndex][0]+=(bb_input_MouseX()-(c_PhotonApp.m_prevMouseX))/c_PhotonApp.m_s;
			this.m_sc.m_spheres[c_PhotonApp.m_sphereIndex][1]-=(bb_input_MouseY()-(c_PhotonApp.m_prevMouseY))/c_PhotonApp.m_s;
		}else{
			this.m_sc.m_light[0]+=(bb_input_MouseX()-(c_PhotonApp.m_prevMouseX))/c_PhotonApp.m_s;
			this.m_sc.m_light[0]=bb_math_Max2(bb_math_Min2(this.m_sc.m_light[0],1.4),-1.4);
			this.m_sc.m_light[1]-=(bb_input_MouseY()-(c_PhotonApp.m_prevMouseY))/c_PhotonApp.m_s;
			this.m_sc.m_light[1]=bb_math_Max2(bb_math_Min2(this.m_sc.m_light[1],1.2),-0.4);
		}
		this.m_render.p_Reset();
	}
	c_PhotonApp.m_prevMouseX=((bb_input_MouseX())|0);
	c_PhotonApp.m_prevMouseY=((bb_input_MouseY())|0);
	c_PhotonApp.m_mouseDragging=1;
	return 0;
}
c_PhotonApp.prototype.p_OnUpdate=function(){
	if((bb_input_KeyHit(32))!=0){
		print("reset");
		this.m_render.p_Reset();
	}
	if((bb_input_KeyHit(49))!=0){
		this.m_render.m_lightPhotons=1;
	}
	if((bb_input_KeyHit(49))!=0){
		this.m_render.m_lightPhotons=0;
	}
	if((bb_input_MouseDown(0))!=0){
		if(!((c_PhotonApp.m_mouseDragging)!=0)){
			this.p_GrabObject();
		}else{
			if((c_PhotonApp.m_prevMouseX)!=bb_input_MouseX() || (c_PhotonApp.m_prevMouseY)!=bb_input_MouseY()){
				this.p_DragObject();
			}
		}
	}else{
		c_PhotonApp.m_prevMouseX=-9999;
		c_PhotonApp.m_prevMouseY=-9999;
		c_PhotonApp.m_mouseDragging=0;
	}
	this.m_render.p_DrawScene(this.m_sc.m_szImg,this.m_sc.m_szImg+48,c_PhotonApp.m_mouseDragging);
	return 0;
}
c_PhotonApp.prototype.p_OnRender=function(){
	bb_graphics_Cls(0.0,0.0,0.0);
	bb_graphics_SetColor(255.0,255.0,255.0);
	bb_graphics_DrawImage(this.m_render.m_img,0.0,0.0,0);
	return 0;
}
...
function bbMain(){
	var t_myApp=c_PhotonApp.m_new.call(new c_PhotonApp);
	return 0;
}
...
c_Image.prototype.p_WritePixels=function(t_pixels,t_x,t_y,t_width,t_height,t_offset,t_pitch,t_frame){
	if(!((t_pitch)!=0)){
		t_pitch=t_width;
	}
	bb_graphics_device.WritePixels2(this.m_surface,t_pixels,this.m_frames[t_frame].m_x+t_x,this.m_frames[t_frame].m_y+t_y,t_width,t_height,t_offset,t_pitch);
	return 0;
}
...
function c_PhotonRender(){
	Object.call(this);
	this.m_view3D=false;
	this.m_lightPhotons=0;
	this.m_empty=1;
	this.m_raytracer=null;
	this.m_width=0;
	this.m_height=0;
	this.m_pRow=0;
	this.m_pCol=0;
...
c_PhotonRender.m_new=function(){
	this.m_view3D=false;
	this.m_lightPhotons=1;
	this.m_empty=1;
	this.m_raytracer=c_RayTracer.m_new.call(new c_RayTracer);
	return this;
}
c_PhotonRender.prototype.p_Reset=function(){
	this.m_pRow=0;
	this.m_pCol=0;
	this.m_pIteration=1;
	this.m_pMax=8;
	this.m_empty=1;
	if(((this.m_lightPhotons)!=0) && !this.m_view3D){
		this.m_raytracer.p_EmitPhotons(this.m_view3D);
	}
	this.m_ms=bb_app_Millisecs();
	return 0;
}
c_PhotonRender.prototype.p_SetScene=function(t_sc,t_w,t_h){
	this.m_width=t_w;
	this.m_height=t_h;
	this.m_raytracer.p_SetScene2(t_sc);
	this.p_Reset();
	this.m_img=bb_graphics_CreateImage(this.m_width,this.m_height,1,c_Image.m_DefaultFlags);
	return 0;
}
c_PhotonRender.prototype.p_Render=function(t_quick){
	var t_x=0;
	var t_y=0;
	var t_tindex=0;
	var t_pxl=0;
	...
	this.m_pRow+=1;
	this.m_pCol=0;
	if(this.m_pRow>=this.m_pMax){
		this.m_pIteration+=1;
		this.m_pRow=0;
		this.m_pMax=((Math.pow(2.0,(this.m_pIteration)))|0);
	}
	t_pmaxsize=((this.m_width/this.m_pMax)|0);
	t_y=this.m_pRow*t_pmaxsize;
	if(t_pmaxsize==0){
		return -1;
	}
	for(var t_px2=0;t_px2<=this.m_pMax;t_px2=t_px2+1){
		var t_pNeedDrawing=0;
		if(this.m_pIteration==1 || ((bb_raytracer_odd(this.m_pRow))!=0) || !((bb_raytracer_odd(this.m_pRow))!=0) && ((bb_raytracer_odd(t_px2))!=0)){
			t_pNeedDrawing=1;
		}
		t_x=t_px2*t_pmaxsize;
		if((t_pNeedDrawing)!=0){
			t_pNeedDrawing=0;
			this.m_iterations+=1;
			t_rgb=this.m_raytracer.p_ComputePixelColor((t_x),(t_y),this.m_lightPhotons);
			var t_psize=t_pmaxsize;
			var t_m=0;
			var t_cc=new_number_array(t_psize*t_psize+t_psize);
			for(var t_n=0;t_n<=t_psize*t_psize;t_n=t_n+1){
				t_cc[t_n]=t_rgb;
			}
			this.m_img.p_WritePixels(t_cc,t_x,t_y,t_psize,t_psize,0,0,0);
		}
	}
	if(this.m_pRow==this.m_height-1){
		this.m_empty=0;
	}
	this.m_iterations=0;
	return 1;
}
c_PhotonRender.prototype.p_DrawScene=function(t_w,t_h,t_quick){
	this.m_width=t_w;
	this.m_height=t_h;
	if(this.m_view3D){
		if((this.m_empty)!=0){
		...
		if((this.m_empty)!=0){
			if(this.p_Render(t_quick)>0){
				this.m_xx+=1;
				if(this.m_xx>this.m_pRow){
					this.m_xx=0;
					this.m_yy+=1;
					print(String(this.m_pMax)+" "+String(this.m_pRow)+" "+String(bb_app_Millisecs()-this.m_ms));
				}
			}
		}
	}
...
function c_RayTracer(){
	Object.call(this);
	this.m_photons=null;
	this.m_sc=null;
}
c_RayTracer.m_new=function(){
	this.m_photons=c_Photons.m_new.call(new c_Photons);
	this.m_photons.p_SetRaytracer(this);
	return this;
}
c_RayTracer.prototype.p_SetScene2=function(t_s){
	this.m_sc=t_s;
	return 0;
}
c_RayTracer.prototype.p_RaySphere=function(t_idx,t_r,t_o){
	var t_s=bb_raytracer_sub3(this.m_sc.m_spheres[t_idx],t_o);
	var t_radius=this.m_sc.m_spheres[t_idx][3];
	var t_bounce=null;
	var t_A=bb_raytracer_dot3(t_r,t_r);
	var t_B=-2.0*bb_raytracer_dot3(t_s,t_r);
	var t_C=bb_raytracer_dot3(t_s,t_s)-t_radius*t_radius;
	...
	if(t_D>0.0){
		...
		var t_lDist=(-t_B+t_sign*Math.sqrt(t_D))/(2.0*t_A);
		return t_lDist;
	}
	return 0.0;
}
c_RayTracer.prototype.p_RayPlane=function(t_idx,t_r,t_o){
	var t_axis=((this.m_sc.m_planes[t_idx][0])|0);
	if(t_r[t_axis]!=0.0){
		var t_lDist=(this.m_sc.m_planes[t_idx][1]-t_o[t_axis])/t_r[t_axis];
		return t_lDist;
	}
	return 0.0;
}
c_RayTracer.prototype.p_Raytrace=function(t_ray,t_origin,t_oldpoint){
	var t_bounce=c_BounceType.m_new.call(new c_BounceType);
	t_bounce.m_point=t_oldpoint;
	t_bounce.m_intersect=0;
	var t_dist=0.0;
	t_bounce.m_dist=999999.9;
	for(var t_t=0;t_t<2;t_t=t_t+1){
		for(var t_idx=0;t_idx<this.m_sc.m_nrObjects[t_t];t_idx=t_idx+1){
			if(t_t==0){
				t_dist=this.p_RaySphere(t_idx,t_ray,t_origin);
			}else{
				t_dist=this.p_RayPlane(t_idx,t_ray,t_origin);
			}
			if(t_dist<t_bounce.m_dist && t_dist>0.0){
				t_bounce.m_dist=t_dist;
				t_bounce.m_index=t_idx;
				t_bounce.m_iType=t_t;
				t_bounce.m_intersect=1;
			}
		}
	}
	return t_bounce;
}
c_RayTracer.m_FilterColor=function(t_rgbIn,t_r,t_g,t_b){
	var t_rgbOut=[t_r,t_g,t_b];
	for(var t_c=0;t_c<=2;t_c=t_c+1){
		t_rgbOut[t_c]=bb_math_Min2(t_rgbOut[t_c],t_rgbIn[t_c]);
	}
	return t_rgbOut;
}
c_RayTracer.prototype.p_GetColorRGB=function(t_rgbIn,t_Typ,t_index){
	var t_c0=(this.m_sc.m_colors[t_Typ][t_index]&255)*0.0039215686274509803921568627451;
	var t_c1=((this.m_sc.m_colors[t_Typ][t_index]&65280)>>8)*0.0039215686274509803921568627451;
	var t_c2=((this.m_sc.m_colors[t_Typ][t_index]&16711680)>>16)*0.0039215686274509803921568627451;
	return c_RayTracer.m_FilterColor(t_rgbIn,t_c0,t_c1,t_c2);
}
c_RayTracer.prototype.p_SphereNormal=function(t_idx,t_P){
	return bb_raytracer_normalize3(bb_raytracer_sub3(t_P,this.m_sc.m_spheres[t_idx]));
}
c_RayTracer.prototype.p_PlaneNormal=function(t_idx,t_P,t_O){
	var t_axis=((this.m_sc.m_planes[t_idx][0])|0);
	var t_N=[0.0,0.0,0.0];
	t_N[t_axis]=t_O[t_axis]-this.m_sc.m_planes[t_idx][1];
	return bb_raytracer_normalize3(t_N);
}
c_RayTracer.prototype.p_SurfaceNormal=function(t_Typ,t_index,t_P,t_Inside){
	if(t_Typ==0){
		return this.p_SphereNormal(t_index,t_P);
	}else{
		return this.p_PlaneNormal(t_index,t_P,t_Inside);
	}
}
c_RayTracer.prototype.p_Reflect=function(t_ray,t_fromPoint,t_t,t_i,t_gPoint){
	var t_N=this.p_SurfaceNormal(t_t,t_i,t_gPoint,t_fromPoint);
	return bb_raytracer_normalize3(bb_raytracer_sub3(t_ray,bb_raytracer_mul3c(t_N,2.0*bb_raytracer_dot3(t_ray,t_N))));
}
c_RayTracer.prototype.p_EmitPhotons=function(t_view3d){
	this.m_photons.p_EmitPhotons2(this.m_sc,t_view3d);
	return 0;
}
c_RayTracer.prototype.p_LightDiffuse=function(t_N,t_P){
	var t_L=bb_raytracer_normalize3(bb_raytracer_sub3(this.m_sc.m_light,t_P));
	return bb_raytracer_dot3(t_N,t_L);
}
c_RayTracer.prototype.p_LightObject=function(t_typ,t_idx,t_P,t_lightAmbient){
	var t_i=this.p_LightDiffuse(this.p_SurfaceNormal(t_typ,t_idx,t_P,this.m_sc.m_light),t_P);
	return bb_math_Min2(1.0,bb_math_Max2(t_i,t_lightAmbient));
}
c_RayTracer.prototype.p_ComputePixelColor=function(t_x,t_y,t_lightPhotons){
	if(!((this.m_sc)!=null)){
	...
	var t_bounce=c_BounceType.m_new.call(new c_BounceType);
	var t_intrgb=0;
	var t_rgb=[0.0,0.0,0.0];
	var t_ray=[t_x/(this.m_sc.m_szImg)-0.5,-(t_y/(this.m_sc.m_szImg)-0.5),1.0];
	t_bounce=this.p_Raytrace(t_ray,this.m_sc.m_gOrigin,[0.0,0.0,0.0]);
	if((t_bounce.m_intersect)!=0){
		t_bounce.m_point=bb_raytracer_mul3c(t_ray,t_bounce.m_dist);
		if(t_bounce.m_iType==0 && t_bounce.m_index==1){
			t_ray=this.p_Reflect(t_ray,this.m_sc.m_gOrigin,t_bounce.m_iType,t_bounce.m_index,t_bounce.m_point);
			t_bounce=this.p_Raytrace(t_ray,t_bounce.m_point,t_bounce.m_point);
			if((t_bounce.m_intersect)!=0){
				t_bounce.m_point=bb_raytracer_add3(bb_raytracer_mul3c(t_ray,t_bounce.m_dist),t_bounce.m_point);
			}
		}
		if((t_lightPhotons)!=0){
			t_rgb=this.m_photons.p_GatherPhotons(this.m_sc,t_bounce.m_point,t_bounce.m_iType,t_bounce.m_index);
		}else{
			var t_tType=t_bounce.m_iType;
			var t_tIndex=t_bounce.m_index;
			var t_i=0.2;
			t_bounce=this.p_Raytrace(bb_raytracer_sub3(t_bounce.m_point,this.m_sc.m_light),this.m_sc.m_light,t_bounce.m_point);
			if(t_tType==t_bounce.m_iType && t_tIndex==t_bounce.m_index){
				t_i=this.p_LightObject(t_bounce.m_iType,t_bounce.m_index,t_bounce.m_point,0.2);
			}else{
				var t_bounce2=c_BounceType.m_new.call(new c_BounceType);
				t_i=this.p_LightObject(t_bounce.m_iType,t_bounce.m_index,t_bounce.m_point,0.1);
				for(var t_j=0;t_j<=3;t_j=t_j+1){
					t_ray=bb_raytracer_sub3(t_bounce.m_point,this.m_sc.m_light);
					var t_lightpoint=bb_raytracer_mul3c(this.m_sc.m_light,1.0+bb_random_Rnd()*0.1);
					t_bounce2=this.p_Raytrace(t_ray,t_lightpoint,t_bounce.m_point);
					if(t_bounce2.m_iType==0){
					}
				}
			}
			t_rgb[0]=t_i;
			t_rgb[1]=t_i;
			t_rgb[2]=t_i;
			t_rgb=this.p_GetColorRGB(t_rgb,t_tType,t_tIndex);
		}
	}
	var t_red=((t_rgb[2]*255.0)|0);
	if(t_red<0){
		t_red=0;
	}
	if(t_red>255){
	...
	t_intrgb=-16777216|t_red<<16|t_grn<<8|t_blu;
	return t_intrgb;
}
function c_Photons(){
	Object.call(this);
	this.m_photons=[];
	this.m_numPhotons=[[0,0],[0,0,0,0,0]];
}
c_Photons.m_new=function(){
	this.m_photons=bb_raytracer_AllocateFloatArray2(3,6,1002,4,3);
	return this;
}
c_Photons.m_raytracer=null;
c_Photons.prototype.p_SetRaytracer=function(t_r){
	c_Photons.m_raytracer=t_r;
	return 0;
}
c_Photons.prototype.p_StorePhoton=function(t_Typ,t_id,t_location,t_direction,t_energy){
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][0][0]=t_location[0];
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][0][1]=t_location[1];
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][0][2]=t_location[2];
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][1][0]=t_direction[0];
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][1][1]=t_direction[1];
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][1][2]=t_direction[2];
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][2][0]=t_energy[0];
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][2][1]=t_energy[1];
	this.m_photons[t_Typ][t_id][this.m_numPhotons[t_Typ][t_id]][2][2]=t_energy[2];
	this.m_numPhotons[t_Typ][t_id]+=1;
	if(this.m_numPhotons[t_Typ][t_id]>1000){
		this.m_numPhotons[t_Typ][t_id]=1000;
	}
	return 0;
}
c_Photons.m_shadow=[];
c_Photons.prototype.p_ShadowPhoton=function(t_ray,t_bounce){
	var t_bumpedPoint=bb_raytracer_add3(t_bounce.m_point,bb_raytracer_mul3c(t_ray,0.00001));
	var t_sBounce=c_Photons.m_raytracer.p_Raytrace(t_ray,t_bumpedPoint,t_bounce.m_point);
	var t_shadowPoint=bb_raytracer_add3(bb_raytracer_mul3c(t_ray,t_sBounce.m_dist),t_bumpedPoint);
	this.p_StorePhoton(t_sBounce.m_iType,t_sBounce.m_index,t_shadowPoint,t_ray,c_Photons.m_shadow);
	return 0;
}
c_Photons.prototype.p_EmitPhotons2=function(t_sc,t_view3D){
	if(!((t_sc)!=null)){
		return 0;
	}
	var t_bounce=c_BounceType.m_new.call(new c_BounceType);
	bb_random_Seed=0;
	for(var t_t=0;t_t<2;t_t=t_t+1){
		for(var t_i=0;t_i<t_sc.m_nrObjects[t_t];t_i=t_i+1){
			this.m_numPhotons[t_t][t_i]=0;
		}
	}
	var t_test=1000;
	if(t_view3D){
	...
	for(var t_i2=0;t_i2<=t_test-1;t_i2=t_i2+1){
		var t_bounces=1;
		var t_rgb=[1.0,1.0,1.0];
		var t_ray=bb_raytracer_normalize3(bb_raytracer_rand3(1.0));
		var t_prevPoint=t_sc.m_light;
		while(t_prevPoint[1]>=t_sc.m_light[1]){
			t_prevPoint=bb_raytracer_add3(t_sc.m_light,bb_raytracer_mul3c(bb_raytracer_normalize3(bb_raytracer_rand3(1.0)),0.75));
		}
		if(bb_math_Abs2(t_prevPoint[0])>1.5 || bb_math_Abs2(t_prevPoint[1])>1.2 || ((bb_raytracer_gatedSqDist3(t_prevPoint,t_sc.m_spheres[0],t_sc.m_spheres[0][3]*t_sc.m_spheres[0][3]))!=0.0)){
			t_bounces=4;
		}
		t_bounce=c_Photons.m_raytracer.p_Raytrace(t_ray,t_prevPoint,t_bounce.m_point);
		while(((t_bounce.m_intersect)!=0) && t_bounces<=3){
			t_bounce.m_point=bb_raytracer_add3(bb_raytracer_mul3c(t_ray,t_bounce.m_dist),t_prevPoint);
			t_rgb=bb_raytracer_mul3c(c_Photons.m_raytracer.p_GetColorRGB(t_rgb,t_bounce.m_iType,t_bounce.m_index),1.0/Math.sqrt(t_bounces));
			this.p_StorePhoton(t_bounce.m_iType,t_bounce.m_index,t_bounce.m_point,t_ray,t_rgb);
			this.p_ShadowPhoton(t_ray,t_bounce);
			t_ray=c_Photons.m_raytracer.p_Reflect(t_ray,t_prevPoint,t_bounce.m_iType,t_bounce.m_index,t_bounce.m_point);
			t_bounce=c_Photons.m_raytracer.p_Raytrace(t_ray,t_bounce.m_point,t_bounce.m_point);
			t_prevPoint=t_bounce.m_point;
			t_bounces+=1;
		}
	}
	return 0;
}
c_Photons.prototype.p_GatherPhotons=function(t_sc,t_p,t_Typ,t_id){
	var t_energy=[0.0,0.0,0.0];
	var t_N=c_Photons.m_raytracer.p_SurfaceNormal(t_Typ,t_id,t_p,t_sc.m_gOrigin);
	for(var t_i=0;t_i<this.m_numPhotons[t_Typ][t_id];t_i=t_i+1){
		var t_pt=[this.m_photons[t_Typ][t_id][t_i][0][0],this.m_photons[t_Typ][t_id][t_i][0][1],this.m_photons[t_Typ][t_id][t_i][0][2]];
		var t_gSqu=bb_raytracer_gatedSqDist3(t_p,t_pt,0.8);
		if((t_gSqu)!=0.0){
			var t_ptd=[this.m_photons[t_Typ][t_id][t_i][1][0],this.m_photons[t_Typ][t_id][t_i][1][1],this.m_photons[t_Typ][t_id][t_i][1][2]];
			var t_pte=[this.m_photons[t_Typ][t_id][t_i][2][0],this.m_photons[t_Typ][t_id][t_i][2][1],this.m_photons[t_Typ][t_id][t_i][2][2]];
			var t_weight=bb_math_Max2(0.0,-bb_raytracer_dot3(t_N,t_ptd));
			t_weight*=(1.0-Math.sqrt(t_gSqu))/50.0;
			t_energy=bb_raytracer_add3(t_energy,bb_raytracer_mul3c(t_pte,t_weight));
		}
	}
	return t_energy;
}
function bb_raytracer_AllocateFloatArray(t_i,t_j){
...
function bb_raytracer_AllocateFloatArray2(t_i,t_j,t_k,t_l,t_m){
	var t_arr=new_array_array(t_i);
	for(var t_ind=0;t_ind<t_i;t_ind=t_ind+1){
		t_arr[t_ind]=new_array_array(t_j);
		for(var t_ind2=0;t_ind2<t_j;t_ind2=t_ind2+1){
			t_arr[t_ind][t_ind2]=new_array_array(t_k);
			for(var t_ind3=0;t_ind3<t_k;t_ind3=t_ind3+1){
				t_arr[t_ind][t_ind2][t_ind3]=new_array_array(t_l);
				for(var t_ind4=0;t_ind4<t_l;t_ind4=t_ind4+1){
					t_arr[t_ind][t_ind2][t_ind3][t_ind4]=new_number_array(t_m);
				}
			}
		}
	}
...
function c_Scene(){
	Object.call(this);
	this.m_szImg=512;
	this.m_nrObjects=[2,5];
	this.m_light=[0.0,1.2,3.75];
	this.m_spheres=[[1.0,0.0,4.0,0.5],[-0.6,-1.0,4.5,0.5]];
	this.m_planes=[[0.0,1.5],[1.0,-1.5],[0.0,-1.5],[1.0,1.5],[2.0,5.0]];
	this.m_colors=[[16777215,16777215],[65280,16777215,16711680,16777215,16777215]];
	this.m_gOrigin=[0.0,0.0,0.0];
}
c_Scene.m_new=function(){
	return this;
}
function c_BounceType(){
	Object.call(this);
	this.m_point=[0.0,0.0,0.0];
	this.m_intersect=0;
	this.m_dist=-1.0;
	this.m_index=0;
	this.m_iType=0;
}
c_BounceType.m_new=function(){
	return this;
}
...
function bb_raytracer_rand3(t_s){
	var t_s2=t_s*2.0;
	return [bb_random_Rnd()*t_s2-t_s,bb_random_Rnd()*t_s2-t_s,bb_random_Rnd()*t_s2-t_s];
}
function bb_raytracer_dot3(t_a,t_b){
	return t_a[0]*t_b[0]+t_a[1]*t_b[1]+t_a[2]*t_b[2];
}
function bb_raytracer_mul3c(t_a,t_c){
	var t_result=[t_c*t_a[0],t_c*t_a[1],t_c*t_a[2]];
	return t_result;
}
function bb_raytracer_normalize3(t_v){
	var t_L=Math.sqrt(bb_raytracer_dot3(t_v,t_v));
	return bb_raytracer_mul3c(t_v,1.0/t_L);
}
function bb_raytracer_add3(t_a,t_b){
	var t_result=[t_a[0]+t_b[0],t_a[1]+t_b[1],t_a[2]+t_b[2]];
	return t_result;
}
...
function bb_raytracer_gatedSqDist3(t_a,t_b,t_sqradius){
	var t_c=t_a[0]-t_b[0];
	var t_d=t_c*t_c;
	if(t_d>t_sqradius){
		return 0.0;
...
function bb_raytracer_sub3(t_a,t_b){
	var t_result=[t_a[0]-t_b[0],t_a[1]-t_b[1],t_a[2]-t_b[2]];
	return t_result;
}
...
function bbInit(){
	...
	c_Photons.m_raytracer=null;
	bb_random_Seed=1234;
	c_Photons.m_shadow=[-0.15,-0.15,-0.15];
	c_PhotonApp.m_mouseDragging=0;
	c_PhotonApp.m_sphereIndex=-1;
	c_PhotonApp.m_s=130.0;
	c_PhotonApp.m_prevMouseX=-9999;
	c_PhotonApp.m_prevMouseY=-9999;
}
//${TRANSCODE_END}
